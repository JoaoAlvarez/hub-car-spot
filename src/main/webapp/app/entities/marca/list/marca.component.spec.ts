...
import { sampleWithRequiredData } from '../marca.test-samples';
...
describe('Marca Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.marcas?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });

  describe('trackId', () => {
    it('Should forward to marcaService', () => {
      const entity = { id: 'ABC' };
      jest.spyOn(service, 'getMarcaIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getMarcaIdentifier).toHaveBeenCalledWith(entity);
