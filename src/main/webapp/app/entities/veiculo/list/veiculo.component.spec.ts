...
import { sampleWithRequiredData } from '../veiculo.test-samples';
...
describe('Veiculo Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.veiculos?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });

  describe('trackId', () => {
    it('Should forward to veiculoService', () => {
      const entity = { id: 'ABC' };
      jest.spyOn(service, 'getVeiculoIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getVeiculoIdentifier).toHaveBeenCalledWith(entity);
