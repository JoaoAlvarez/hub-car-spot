...
import { EntityArrayResponseType, VeiculoService } from '../service/veiculo.service';
...
export class VeiculoComponent implements OnInit {
  ...
  trackId = (_index: number, item: IVeiculo): string => this.veiculoService.getVeiculoIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.veiculos = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IVeiculo[] | null): IVeiculo[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.veiculoService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
