...
import { sampleWithRequiredData } from '../instituicao.test-samples';
...
describe('Instituicao Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.instituicaos?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });

  describe('trackId', () => {
    it('Should forward to instituicaoService', () => {
      const entity = { id: 'ABC' };
      jest.spyOn(service, 'getInstituicaoIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getInstituicaoIdentifier).toHaveBeenCalledWith(entity);
